...
BezierTireMachining::BezierTireMachining(const std::string input_mesh) :
    BezierTireMachining(input_mesh, 0, 0, 0, 0, Y, Eigen::Vector3d::Identity())
{
}
                                               ...
                                               const bool display_markers)
{
  ...
  for (vtkSmartPointer<vtkStripper> stripper : grinding_strippers)
  {
    ...
    {
      ROS_ERROR_STREAM(
          "BezierTireMachining::generateTrajectory: Grinding stripper has " << stripper->GetOutput()->GetNumberOfLines() << " lines");
      ...
      it != grinding_strippers.end(); ++it)
  {
    ...
    {
      ROS_WARN_STREAM("BezierTireMachining::generateTrajectory: Could not generate robot poses for grinding trajectory");
      continue;
    }

    if (harmonizeLineOrientation(traj, direction_reference))
      ROS_INFO_STREAM("BezierTireMachining::generateTrajectory: Grinding line reversed");
...
std::string BezierTireMachining::validateParameters()
{
